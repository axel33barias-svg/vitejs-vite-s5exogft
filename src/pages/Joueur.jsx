...
export default function Joueur() {
  ...
  // Enregistre le joueur et écoute ses modifs en temps réel
  useEffect(() => {
    if (!nomConfirme || !sessionId) return;

    const init = async () => {
      // Crée le joueur dans la table
      const { data } = await supabase
        .from("joueurs")
  ...
  // Écoute les changements de modifs par le MJ
  useEffect(() => {
    if (!joueurId) return;
  ...
  const lancerDe = useCallback(async (faces) => {
    ...
    setTimeout(async () => {
      ...
      await supabase.from("lancers").insert([{
        ...
        session_id: sessionId, auteur: nom
      }]);
    }, 400);
  ...
  if (sessionValide === false) return (
    <div style={{
      ...
      fontFamily: "sans-serif", color: "white", gap: 12,
    }}>
  ...
  if (!nomConfirme) return (
    <div style={{
      ...
      <div style={{
        maxWidth: 380, width: "90%", background: "#16213e",
        padding: 30, borderRadius: 15, border: "1px solid #0f3460",
        ...
        <input
          ...
          style={{
            width: "100%", background: "#1a1a2e", border: "1px solid #e94560",
            color: "white", padding: "12px", borderRadius: 8, fontSize: "1rem",
            boxSizing: "border-box", outline: "none", marginBottom: 14, textAlign: "center",
          }}
        />
        <button
          onClick={() => nom.trim() && setNomConfirme(true)}
          disabled={!nom.trim()}
          style={{
            width: "100%", background: nom.trim() ? "#e94560" : "#0f3460",
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          padding: "5px 14px", borderRadius: 20, fontSize: 13, fontWeight: "bold",
        }}>
          ⚔️ {nom}
        </span>
      </div>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "24px 16px" }}>
        ...
        {/* Modifs définis par le MJ — lecture seule */}
        <div style={{
          ...
          {[
            { label: "Modificateur", value: bonus },
            { label: "Seuil", value: seuil },
          ].map(({ label, value }) => (
            <div key={label} style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "center" }}>
              ...
              <div style={{
                background: "#1a1a2e", border: "1px solid #0f3460", color: "#95a5a6",
        ...
        <div style={{
          ...
          <div style={{
            fontSize: 52, fontWeight: "bold", lineHeight: 1, color: resultColor,
            opacity: rolling ? 0.3 : 1, transition: "all 0.3s",
          }}>
          ...
          {lastRoll && !rolling && (
            <>
              ...
              {lastRoll.status && (
                <div style={{ marginTop: 8, fontWeight: "bold", color: resultColor, fontSize: 15 }}>
                  {lastRoll.status.label}
                </div>
              )}
            </>
          )}
          {!lastRoll && !rolling && (
            <div style={{ color: "#95a5a6", fontSize: 14 }}>Lancez un dé !</div>
          )}
        </div>
        ...
        {history.length > 0 && (
          <div>
            <h3 style={{ fontSize: "0.9rem", color: "#95a5a6", textTransform: "uppercase", letterSpacing: 1, marginTop: 0 }}>
              Mes lancers
            </h3>
            <div style={{ background: "#16213e", border: "1px solid #0f3460", borderRadius: 10, maxHeight: 200, overflowY: "auto" }}>
              {history.map((r) => (
                <div key={r.id} style={{
                  ...
                  padding: "8px 14px", borderBottom: "1px solid #0f3460",
                }}>
